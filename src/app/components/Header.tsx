import { Link } from "react-router";
import { Menu, X, Download } from "lucide-react";
import { useState } from "react";
import { Logo } from "./Logo";
import { Button } from "./Button";

const NAV_LINKS = [
  { to: "/products/quickshottransfer", label: "Products" },
  { to: "/pricing", label: "Pricing" },
  { to: "/download", label: "Download" },
  { to: "/support", label: "Support" },
] as const;

export function Header() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-700/50 bg-gray-900/60 backdrop-blur-xl">
      <nav
        className="mx-auto grid max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-x-4 px-6 py-4 lg:px-8"
        aria-label="Global"
      >
        <div className="flex min-w-0 items-center">
          <Logo />
        </div>

        <div className="hidden items-center justify-center gap-x-1 lg:flex">
          {NAV_LINKS.map(({ to, label }) => (
            <Link
              key={to}
              to={to}
              className="rounded-lg px-3 py-2 text-[15px] font-medium text-gray-300 transition-colors hover:bg-gray-800/80 hover:text-white"
            >
              {label}
            </Link>
          ))}
        </div>

        <div className="flex items-center justify-end gap-2">
          <button
            type="button"
            className="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-gray-300 hover:bg-gray-800/80 hover:text-white lg:hidden"
            onClick={() => setMobileMenuOpen(true)}
          >
            <span className="sr-only">Open main menu</span>
            <Menu className="h-6 w-6" aria-hidden="true" />
          </button>
          <div className="hidden lg:block">
            <Button to="/download" variant="primary" size="sm">
              <Download className="h-4 w-4" />
              Download for Windows
            </Button>
          </div>
        </div>
      </nav>

      {mobileMenuOpen ? (
        <div className="lg:hidden" role="dialog" aria-modal="true">
          <div className="fixed inset-0 z-50 bg-slate-900/20 backdrop-blur-sm" />
          <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-gray-900 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-700/50">
            <div className="flex items-center justify-between">
              <div onClick={() => setMobileMenuOpen(false)}>
                <Logo />
              </div>
              <button
                type="button"
                className="-m-2.5 rounded-lg p-2.5 text-gray-300 hover:bg-gray-800"
                onClick={() => setMobileMenuOpen(false)}
              >
                <span className="sr-only">Close menu</span>
                <X className="h-6 w-6" aria-hidden="true" />
              </button>
            </div>
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-gray-700/60">
                <div className="space-y-2 py-6">
                  {NAV_LINKS.map(({ to, label }) => (
                    <Link
                      key={to}
                      to={to}
                      className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-white hover:bg-gray-800"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      {label}
                    </Link>
                  ))}
                </div>
                <div className="py-6">
                  <Button to="/download" variant="primary" size="md" className="w-full">
                    <Download className="h-4 w-4" />
                    Download for Windows
                  </Button>
                </div>
              </div>
            </div>
          </div>
        </div>
      ) : null}
    </header>
  );
}
